...
public class BytecodeGenListener extends MiniCBaseListener implements ParseTreeListener {
	...
	public void exitWhile_stmt(MiniCParser.While_stmtContext ctx) {
		...
		String lend = symbolTable.newLabel(); //end Label
		String lLoop = symbolTable.newLabel(); //Loop Label
...
//		fun_decl	: type_spec IDENT '(' params ')' compound_stmt ;
		...
		String fHeader = funcHeader(ctx, ctx.IDENT().getText());
		if (isVoidF(ctx)) {
			if (!newTexts.get(ctx.compound_stmt().getChild(ctx.compound_stmt().getChildCount() - 2))
					.equals("return;")) {
				newTexts.put(ctx, fHeader + newTexts.get(ctx.compound_stmt()) + "return\n" + ".end method\n");
				return;
			}

		}
		newTexts.put(ctx, fHeader + newTexts.get(ctx.compound_stmt()) + ".end method\n");
	...
	public void exitCompound_stmt(MiniCParser.Compound_stmtContext ctx) { 
		...
		newTexts.put(ctx, newTexts.get(ctx) + ex);
